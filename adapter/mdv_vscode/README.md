...
2. Existing archives also open directly in Doc, without switching or revealing your sidebar. There is no overview page. Open **MDV / Versions** from the activity bar when you want it. Click the colored **REF ›** or **DOC ›** title to open and focus that working copy. The adjacent square controls only show or hide each source; **MDV: Show / Hide Ref** and **MDV: Show / Hide Doc** provide the same visibility actions. Ref stays on the left and Doc on the right. Both are native `markdown` editors, without diff highlighting. **MDV: New Document…** remains an optional shortcut.
...
For actual Agent interoperability, pass `--agent-cli /absolute/path/to/installed/cli.cjs` to the installed test runner. It launches the independent Agent tool and checks paired reads, a Document save, clean-editor refresh and dirty-buffer conflict protection. The 2026-09-09 v1 run was **20/21** because the native undo assertion failed. On 2026-09-10, the test was migrated to the v2 tree baseline and rerun with an installed `0.1.0-preview.2` personal runtime: the complete installed-VSIX suite passed **22/22**, including the Agent subprocess case, and renderer-log inspection found no disposed Webview error. [O006](../../docs/design/open-questions.md#o006原生撤销回归在-agent-联合检查中失败) remains as a historical intermittent observation until its cause is understood. No production plugin or Core code changed for this interoperability update.
