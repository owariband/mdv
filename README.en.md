...
`milkdownv` is a quiet desktop editor for `.mdv`: edit Reference and Document side by side, with version history and exact binds kept visible; ordinary `.md` stays single-pane.

<p align="center">
  <img src="docs/assets/promo/milkdownv-writing.gif" alt="milkdownv moves from the full desktop app into editable Reference and Document panes side by side, then returns to the full view" width="920">
</p>

<p align="center"><sub>01 · Write quietly, side by side: Reference and Document remain visible as two editable working copies.</sub></p>

<p align="center">
  <img src="docs/assets/promo/milkdownv-exact-bind.gif" alt="milkdownv focuses on the real Worktree and shows a Document Version bound to an exact earlier Reference Version while Reference HEAD moves on" width="920">
</p>

<p align="center"><sub>02 · See the exact bind: Reference HEAD can move on while an existing Document→Reference bind stays on its original version.</sub></p>

> **Desktop source preview.** It currently builds from source only; there is no installer, signing, or auto-update. The version graph is read-only, with no commit or checkout yet.

[See the capability boundaries and source-build instructions](adapter/mdv_desktop/README.md#local-development)
...
> **MDV is public through its GitHub source.** Format 0.1, Core, the VS Code Plugin, and the Agent Tool are open in this repository. `milkdownv` is also included as a source-build preview, but it does not yet ship an installer, signing, or auto-update. GitHub source remains the public distribution channel.
...
MDV—Markdown Document with Versions—gives a Markdown workflow portable memory, so you do not have to trade away the human original to gain the convenience of AI. It does not require you to replace your editor: keep using VS Code, or choose the focused `milkdownv` in this repository.

## What we ship

MDV is more than a file extension. This repository provides the MDV document model together with four components for developers, people, and Agents; `milkdownv` is currently a source-build preview:

**In one line: Core owns `.mdv` semantics, the VS Code Plugin and `milkdownv` serve people, and the Agent Tool serves Agents.**
...
`milkdownv` requires Node.js 22.12+. It can open files or folders, edit and safely save the Ref and Doc working copies side by side, and inspect existing versions, branches, and binds. The graph is currently read-only, so commit and checkout are not available in the app yet. Ordinary `.md` files use one editor. There is no installer, signing, or auto-update yet. `prepare:core` packs the current local Core into a pinned tarball; `milkdownv` remains an independent npm package.
...
- [milkdownv](adapter/mdv_desktop/README.md) — desktop capability boundaries, local setup, and architecture.
...
`milkdownv` is an independent package and has its own verification commands:
